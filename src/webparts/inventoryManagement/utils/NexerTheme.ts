...
let cachedWhitePng: string | undefined;
let cachedBlackPng: string | undefined;

export const getNexerLogoPng = (variant: 'white' | 'black' = 'white'): Promise<string> => {
      ...
      img.onload = () => {
        try {
          ...
          if (ctx) {
            ...
            if (variant === 'white') cachedWhitePng = pngDataUrl;
            else cachedBlackPng = pngDataUrl;
...
export const generateNexerPdfReport = async (options: INexerPdfReportOptions): Promise<void> => {
  try {
    const doc = new jsPDF();
    ...
    try {
      if (options.reportedDate) formattedReported = new Date(options.reportedDate).toLocaleString();
    } catch {
      // keep raw string
    }
