...
const mockSP = {
  web: {
    ...
    ensureUser: (name: string) => Promise.resolve({ Id: 15, data: { Id: 15 } })
  }
};
...
let inventoryListItems: any[] = [];
let mappingListItems: any[] = [];
let requestListItems: any[] = [];
    ...
    return {
      ...
      items: {
        getById: (id: any) => {
          ...
          getItem.update = async (payload: any) => {
            const item = inventoryListItems.find(i => i.ID === numId);
            if (item) {
...
describe("Asset Assignment Workflow Verification", () => {
  ...
  it("should successfully assign an In Stock asset to an employee", async () => {
    await AssetAssignmentService.assignAssetsToEmployee(
      ["101"],
      "Adele Vance",
      "adele@contoso.com",
      "Admin User",
      "E1001",
      "Dev laptop assignment"
    );
