...
const mockSP = {
  web: {
    lists: {
      getByTitle: (title: string) => getMockList(title),
...
let inventoryListItems = [
  {
    ...
    AssignedTo: "Adele Vance" as string | null,
    AssignedToId: 12 as number | null,
    Condition: "Good",
...
function getMockList(title: string): any {
            ...
            select: () => {
              return () => {
                const match = returnRequestsListItems.filter(item => {
                  if (filterStr.includes("RR-101") || filterStr.includes("#101")) return item.ReturnRequestID === "RR-101";
                  return false;
                });
          ...
          select: () => {
            return () => {
              const match = mappingListItems.filter(item => {
                if (filterStr.includes("DELL12345")) return item.SerialNumber === "DELL12345";
                return false;
              });
              return Promise.resolve(match);
            };
...
describe("Asset Return Two-Level Approval Workflow Verification", () => {
  ...
  it("should successfully execute two-level return approval workflow", async () => {
    ...
    await ReturnRequestService.updateReturnRequestStatus(
      "RR-101",
      "Pending Admin Verification",
      "Return approved by Manager during test.",
      "Loka Kiran Reddy",
      undefined,
      undefined,
      "Approved",
    ...
    await ReturnRequestService.updateReturnRequestStatus(
      ...
      "IT Admin User",
      "Good",
      "Verified checked in by IT admin.",
      "Approved",
      "Completed"
    );
