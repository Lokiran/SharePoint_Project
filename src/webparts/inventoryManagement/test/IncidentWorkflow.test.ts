...
const mockSP = {
  web: {
    ...
    ensureUser: (name: string) => Promise.resolve({ Id: 20, data: { Id: 20 } })
  }
};
...
let incidentListItems: any[] = [];
let inventoryListItems: any[] = [];
let replacementListItems: any[] = [];
...
describe("Incident Management & Replacement Workflow Verification", () => {
  ...
  beforeEach(() => {
    incidentService = new IncidentService({ pageContext: { web: { absoluteUrl: "https://contoso.sharepoint.com" } } } as any);
    incidentListItems = [];
    inventoryListItems = [
      {
        ID: 301,
