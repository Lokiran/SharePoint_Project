    ...
    if (shouldFetchEventLog) {
      try {
        ...
        if (filters.module && filters.module !== 'All') {
          ...
          if (filters.module === 'Inventory') moduleVal = 'Asset';
          else if (filters.module === 'Requests') moduleVal = 'Request';
    ...
    if (shouldFetchRequests) {
      try {
        ...
        const requestItems = await SharePointBaseService._fetchItemsWithExpandedUsers(
          reqList, 
          reqFilters.length > 0 ? reqFilters.join(' and ') : undefined
        );
