...
export class IncidentService {
  ...
  private async _ensureReplacementListFields(): Promise<void> {
  ...
  private _migrateLocalReplacements(): void {
    try {
      const localIncidents = this.getLocalItems('spfx_mock_incidents');
      const replacementsToMove = localIncidents.filter(item => 
        item.incidentType === 'Replacement Request' || 
      ...
      if (replacementsToMove.length > 0) {
        ...
        const remainingIncidents = localIncidents.filter(item => 
          item.incidentType !== 'Replacement Request' && 
          item.issueType !== 'Replacement Request' &&
          item.id !== '384' &&
          item.id !== '976' &&
        ...
        // Filter duplicates
        const uniqueReplacements = updatedReplacements.filter((item, index, self) =>
          self.findIndex(t => t.id === item.id || t.incidentId === item.incidentId) === index
        );
  ...
  public async getEmployeeReplacementHistory(userEmail: string, isAdmin?: boolean): Promise<any[]> {
    let replacementItems: any[] = [];
    let historicalSharePointReplacements: any[] = [];
      ...
      if (isLocal) {
        ...
        try {
          ...
          const getFieldName = (displayNames: string[]) => {
            for (const name of displayNames) {
              const field = fields.find((f: any) => f.Title && f.Title.toLowerCase() === name.toLowerCase());
              if (field) return field.InternalName;
            }
            return null;
          };
          ...
          const payload: any = { [statusField]: status };
  ...
  public async createIncidentRequest(incidentData: any, file?: File): Promise<any> {
    ...
    try {
      ...
      const isForbiddenOrMissing = error.message && (
