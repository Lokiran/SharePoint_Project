...
export class SharePointBaseService {
  ...
  public static async _coerceAndValidatePayload(
    logicalPayload: { [logicalKey: string]: any },
    schema: IFieldMetadata[],
    resolvedMapping: { [logicalKey: string]: string },
    requiredKeys: string[]
  ): Promise<any> {
      ...
      if (fieldType === "DateTime") {
        ...
        if (typeof val === "number") {
          ...
          if (!isNaN(parsedId) && String(parsedId) === valTrim) {
            ...
            try {
              const sp = getSP();
              const user: any = await sp.web.ensureUser(valTrim);
  ...
  public static _resolveFieldInternalName(
    ...
    for (const alias of aliases) {
      const matched = fields.find(f => 
        !excludeFields.has(f.internalName) && 
        (f.internalName.toLowerCase() === alias.toLowerCase() || f.displayName.toLowerCase() === alias.toLowerCase())
      );
      if (matched) return matched.internalName;
    }
    for (const alias of aliases) {
      const normAlias = alias.toLowerCase().replace(/[^a-z0-9]/g, '');
      const matched = fields.find(f => {
        if (excludeFields.has(f.internalName)) return false;
  ...
  public static _isBusinessStatusKey(key: string): boolean {
