...
export class RequestService {
  ...
  private static _extractRequestKey(item: any): string {
    ...
    for (const key of Object.keys(item)) {
      ...
      if (
        ...
        return RequestService._normalizeRequestKey(
          item[key].toString()
        );
      }
    }
    ...
    if (item.ID) {
      return RequestService._buildRequestKeyFromItemId(
    ...
    for (const item of items) {
      try {
        ...
        if (!Number.isNaN(itemId)) {
          const requestKey =
            RequestService._buildRequestKeyFromItemId(itemId);
      ...
      const requestKey =
        Number.isNaN(requestItemId)
          ? `REQ-${Date.now()
              .toString(36)
              .toUpperCase()}`
          : RequestService._buildRequestKeyFromItemId(
              requestItemId
            );
      ...
      if (!Number.isNaN(requestItemId)) {
        try {
          ...
          const resolvedKeyName =
            RequestService._resolveRequestKeyInternalName(
              fields
            );
      ...
      mapped = items.map(
        (item: any) => {
          ...
          const requestKey =
            ...
            RequestService._extractRequestKey(
              item
            );
  ...
  public static async getRequestHistoryById(
    ...
    const normalizedRequestKey =
      RequestService._normalizeRequestKey(
        requestLookupId
      );
