    ...
    try {
      ...
      Promise.resolve().then(async () => {
        try {
          ...
          try {
            const resolvedEmail = await EmailService.resolveLiveManagerEmail(request.managerName || "");
            if (resolvedEmail) {
      ...
      if (status === 'Approved') {
        ...
        try {
          ...
          if (!finalEmployeeEmail) {
            const empRecord = EMPLOYEES.find(e => e.name.toLowerCase() === employeeName.toLowerCase());
            if (empRecord) finalEmployeeEmail = empRecord.email;
          }

          await EmailService.sendRejectionNotificationToEmployee({
            requestKey: requestKey || RequestService._buildRequestKeyFromItemId(requestId),
            employeeName,
