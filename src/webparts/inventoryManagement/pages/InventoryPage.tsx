  ...
  return (isAdmin || isInventoryManager) ? (
    <div>
      <div className={styles.cardHeader}>
        <h3>{strings.InventoryPage.Title}</h3>
      </div>
      <p style={{ color: 'var(--text-muted)', marginBottom: '20px' }}>
        {strings.InventoryPage.Description}
      </p>
      {loading ? (
        <p>{strings.InventoryPage.LoadingInventory}</p>
      ) : (
        <div>
          <div style={{ marginBottom: '15px' }}>
            <PrimaryButton
              text={isAdmin ? strings.InventoryPage.ButtonAddNewAsset : strings.InventoryPage.ButtonAssignManageAssets}
