    ...
    constructor(props) {
        ...
        this._onSendMockEmail = async () => {
            ...
            try {
                ...
                const emailService = new EmailService(this.props.spContext);
    ...
    render() {
        ...
        return (React.createElement("section", { className: `${styles.inventoryManagement} ${hasTeamsContext ? styles.teams : ''} ${isDarkTheme ? styles.dark : ''}` },
                    ...
                    React.createElement("div", { className: `${styles.card} ${styles.contentContainer}` },
                        ...
                        (() => {
                            ...
                            switch (this.state.selectedTabKey) {
                                ...
                                case 'Notifications':
                                    return (React.createElement(NotificationCenter, { notifications: notifications, clearedNotificationIds: this.state.clearedNotificationIds, onMarkAsRead: this._markNotificationAsRead, onMarkAllAsRead: this._markAllNotificationsAsRead, onClearNotification: this._clearNotification, onClearAllNotifications: this._clearAllNotifications, onClearNotifications: this._clearNotifications, onNotificationAction: this._handleNotificationAction }));
            ...
            React.createElement(Panel, { isOpen: this.state.lastMockEmail !== undefined, onDismiss: () => this.setState({ lastMockEmail: undefined }), type: PanelType.medium, headerText: "\uD83D\uDCEC Outgoing Email Notification (Developer Preview)", closeButtonAriaLabel: "Close", onRenderFooterContent: () => (React.createElement(Stack, { horizontal: true, tokens: { childrenGap: 10 }, style: { padding: '10px 0' } },
                ...
                this.state.mockEmailSendSuccess && this.state.mockEmailSendMethod === "graph" && (React.createElement(MessageBar, { messageBarType: MessageBarType.success }, "Email sent via Microsoft Graph.")),
                this.state.mockEmailSendSuccess && this.state.mockEmailSendMethod === "spUtility" && (React.createElement(MessageBar, { messageBarType: MessageBarType.warning }, "Queued via legacy SharePoint mail - please verify delivery.")),
