        ...
        ...((isManager || isAdmin) ? [
            {
                ...
                onRender: (item) => {
                    ...
                    if (item.status !== 'Pending') {
                        return (React.createElement(Stack, { horizontal: true, tokens: { childrenGap: 6 }, verticalAlign: "center" },
                            viewButton,
                            item.status === 'Approved' && (React.createElement(PrimaryButton, { text: "Verify & Complete", onClick: () => openDialog(item, 'Complete'), styles: { root: { height: 26, padding: '4px 8px', fontSize: '0.75rem', backgroundColor: '#107c41', borderColor: '#107c41' } } }))));
                    }
    ...
    return (React.createElement("div", { style: { marginTop: '10px' } },
        ...
        React.createElement(Dialog, { hidden: !activeRequest, onDismiss: closeDialog, dialogContentProps: {
            ...
            React.createElement(Stack, { tokens: { childrenGap: 15 }, style: { marginTop: '15px' } },
                ...
                actionType !== 'View' && (React.createElement(TextField, { label: actionType === 'Reject' ? 'Rejection Reason (Required)' : 'Manager Comments / Verification Notes', placeholder: actionType === 'Reject' ? 'Please specify why this return request is being rejected...' : 'Add check-in details (e.g. checked power cords, verified serial number...)', multiline: true, rows: 3, value: comment, onChange: (_, val) => setComment(val || ''), required: actionType === 'Reject' }))),
