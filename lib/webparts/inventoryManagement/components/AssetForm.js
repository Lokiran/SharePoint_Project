...
export const AssetForm = (props) => {
    ...
    return (React.createElement(Panel, { isOpen: props.isOpen, onDismiss: props.onClose, type: PanelType.custom, customWidth: "100%", styles: { main: { maxWidth: '450px' } }, headerText: isAdmin ? "Add New Asset" : "Register New Asset", closeButtonAriaLabel: "Close" },
        React.createElement(Stack, { tokens: stackTokens },
            ...
            React.createElement(Dropdown, { label: "Asset Type", selectedKey: assetType, options: DEFAULT_ASSET_TYPE_OPTIONS, onChange: (_, opt) => setAssetType(opt?.key || 'Other'), required: true }),
            ...
            React.createElement(Dropdown, { label: "Condition", selectedKey: condition, options: ASSET_CONDITION_OPTIONS, onChange: (_, opt) => setCondition(opt?.key || 'New') }),
