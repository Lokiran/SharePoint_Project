...
export const RequestForm = (props) => {
    ...
    return (React.createElement(Panel, { isOpen: props.isOpen, onDismiss: props.onClose, type: PanelType.custom, customWidth: "100%", styles: { main: { maxWidth: '450px' } }, headerText: strings.RequestForm.HeaderText, closeButtonAriaLabel: strings.Common.Close },
        React.createElement(Stack, { tokens: stackTokens },
            React.createElement(MessageBar, { messageBarType: MessageBarType.info }, strings.RequestForm.OnBehalfHint),
            React.createElement(Dropdown, { label: strings.RequestForm.LabelRequester, selectedKey: selectedRequesterId, options: employeeOptions, required: true, disabled: true }),
            React.createElement(TextField, { label: strings.RequestForm.LabelEmployeeId, value: employeeId, onChange: (_, val) => setEmployeeId(val || ''), required: true, disabled: activeEmployee.id !== 'current-user' }),
            React.createElement(TextField, { label: strings.RequestForm.LabelManagerName, value: managerName, onChange: (_, val) => {
                    setManagerName(val || '');
            ...
            React.createElement(TextField, { label: strings.RequestForm.LabelRequestedDate, type: "date", value: requestDate, onChange: (_, val) => setRequestDate(val || ''), required: true }),
            React.createElement(Dropdown, { label: strings.RequestForm.LabelAssetType, selectedKey: selectedAssetType, options: assetTypeOptions, onChange: (_, opt) => {
                    setSelectedAssetType(opt?.key);
                }, required: true }),
            React.createElement(Dropdown, { label: strings.RequestForm.LabelPriority, selectedKey: priority, options: ASSET_REQUEST_PRIORITY_OPTIONS, onChange: (_, opt) => setPriority(opt?.key), required: true }),
            React.createElement(TextField, { label: strings.RequestForm.LabelQuantity, type: "number", value: quantity.toString(), onChange: (_, val) => setQuantity(parseInt(val || '0')), required: true }),
            React.createElement(TextField, { label: strings.RequestForm.LabelReason, multiline: true, rows: 3, value: reason, onChange: (_, val) => {
                    setReason(val || '');
            ...
            React.createElement(Stack, { horizontal: true, tokens: stackTokens, style: { marginTop: 20 } },
                React.createElement(PrimaryButton, { text: strings.RequestForm.SubmitRequest, onClick: onSave, disabled: !isFormValid }),
