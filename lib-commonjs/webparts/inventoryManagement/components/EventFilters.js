...
const EventFilters = (props) => {
    ...
    return (React.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '20px' } },
        React.createElement("div", { style: { display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' } },
            React.createElement(react_1.SearchBox, { placeholder: "Search by asset name, title, details, user, or action...", value: filters.searchQuery, onChange: (_, newValue) => onChange({ ...filters, searchQuery: newValue || '' }), onClear: () => onChange({ ...filters, searchQuery: '' }), styles: { root: { flexGrow: 1, minWidth: '300px' } } }),
            React.createElement(react_1.DefaultButton, { text: "Clear Filters", iconProps: { iconName: 'ClearFilter' }, onClick: onClear, disabled: !hasActiveFilters && !filters.searchQuery })),
        React.createElement("div", { style: {
                display: 'grid',
                gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))',
                gap: '12px'
            } },
            React.createElement(react_1.Dropdown, { label: "Date Range", selectedKey: filters.dateRangeType, options: dateOptions, onChange: (_, option) => option && handleDateChange(option.key) }),
            React.createElement(react_1.Dropdown, { label: "Action", selectedKey: filters.action, options: actionOptions, onChange: (_, option) => option && onChange({ ...filters, action: option.key }) }),
            React.createElement(react_1.Dropdown, { label: "Module", selectedKey: filters.module, options: moduleOptions, onChange: (_, option) => option && onChange({ ...filters, module: option.key }) }),
            React.createElement(react_1.Dropdown, { label: "Asset Type", selectedKey: filters.assetType, options: assetTypeOptions, onChange: (_, option) => option && onChange({ ...filters, assetType: option.key }) }),
            React.createElement(react_1.Dropdown, { label: "User", selectedKey: filters.user, options: userOptions, onChange: (_, option) => option && onChange({ ...filters, user: option.key }) }),
            React.createElement(react_1.Dropdown, { label: "Status", selectedKey: filters.status, options: statusOptions, onChange: (_, option) => option && onChange({ ...filters, status: option.key }) }),
            React.createElement(react_1.Dropdown, { label: "Sort Order", selectedKey: filters.sortOrder, options: sortOptions, onChange: (_, option) => option && onChange({ ...filters, sortOrder: option.key }) })),
        filters.dateRangeType === 'Custom' && (React.createElement(react_1.Stack, { horizontal: true, wrap: true, tokens: { childrenGap: 16 }, style: { alignItems: 'flex-end', backgroundColor: '#f3f2f1', padding: '12px', borderRadius: '4px' } },
            React.createElement("div", null,
                React.createElement(react_1.DatePicker, { label: "Start Date", value: filters.startDate, onSelectDate: (date) => date && onChange({ ...filters, startDate: date }), placeholder: "Select a start date..." })),
            React.createElement("div", null,
                React.createElement(react_1.DatePicker, { label: "End Date", value: filters.endDate, onSelectDate: (date) => date && onChange({ ...filters, endDate: date }), placeholder: "Select an end date..." })),
            isDateRangeInvalid && (React.createElement(react_1.Text, { style: { color: '#a80000', alignSelf: 'center', fontWeight: 'bold' } }, "Warning: Start Date must be less than or equal to End Date.")))),
        hasActiveFilters && (React.createElement("div", { style: { display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center', marginTop: '4px' } },
            React.createElement(react_1.Text, { variant: "smallPlus", style: { color: 'var(--text-muted)', marginRight: '4px', fontWeight: 'bold' } }, "Active Filters:"),
            ...
            React.createElement(react_1.DefaultButton, { text: "Clear All", onClick: onClear, styles: { root: { height: 26, minWidth: 0, padding: '0 8px', fontSize: '0.8rem' } } })))));
};
