...
const ReturnRequestList = (props) => {
        ...
        ...((isManager || isAdmin) ? [
            {
                ...
                isResizable: true,
                onRender: (item) => {
                    const viewButton = (React.createElement(Button_1.DefaultButton, { text: strings.Common.View, onClick: () => openDialog(item, 'View'), styles: { root: { height: 26, padding: '4px 8px', fontSize: '0.75rem' } } }));
                    if (isManager && (item.status === 'Pending Manager Approval' || item.status === 'Pending')) {
                        return (React.createElement(Stack_1.Stack, { horizontal: true, tokens: { childrenGap: 6 } },
                            viewButton,
                            React.createElement(Button_1.PrimaryButton, { text: strings.ReturnRequestList.ButtonApprove, onClick: () => openDialog(item, 'Approve'), styles: { root: { height: 26, padding: '4px 8px', fontSize: '0.75rem' } } }),
                            React.createElement(Button_1.DefaultButton, { text: strings.ReturnRequestList.ButtonReject, onClick: () => openDialog(item, 'Reject'), styles: { root: { height: 26, padding: '4px 8px', fontSize: '0.75rem', color: '#b91c1c', borderColor: '#fee2e2' } } })));
                    }
                    if (isAdmin && item.status === 'Pending Admin Verification') {
                        return (React.createElement(Stack_1.Stack, { horizontal: true, tokens: { childrenGap: 6 } },
                            viewButton,
                            React.createElement(Button_1.PrimaryButton, { text: strings.ReturnRequestList.ButtonVerifyComplete, onClick: () => openDialog(item, 'Complete'), styles: { root: { height: 26, padding: '4px 8px', fontSize: '0.75rem', backgroundColor: '#107c41', borderColor: '#107c41' } } })));
                    }
    ...
    return (React.createElement("div", { style: { marginTop: '10px' } },
        React.createElement("div", { style: { marginBottom: '15px' } },
            React.createElement(SearchBox_1.SearchBox, { placeholder: strings.ReturnRequestList.SearchPlaceholder, value: searchQuery, onChange: (_, val) => setSearchQuery(val || ''), styles: { root: { maxWidth: 350 } } })),
        loading ? (React.createElement("p", null, strings.ReturnRequestList.LoadingReturnRequests)) : filteredItems.length === 0 ? (React.createElement("p", { style: { fontStyle: 'italic', color: '#6b7280' } }, strings.ReturnRequestList.EmptyState)) : (React.createElement("div", { className: InventoryManagement_module_scss_1.default.tableWrapper },
        ...
        React.createElement(Dialog_1.Dialog, { hidden: !activeRequest, onDismiss: closeDialog, dialogContentProps: {
            ...
            React.createElement(Dialog_1.DialogFooter, null, actionType !== 'View' ? (React.createElement(React.Fragment, null,
                ...
                React.createElement(Button_1.DefaultButton, { text: strings.Common.Cancel, onClick: closeDialog, disabled: submitting }))) : (React.createElement(Button_1.PrimaryButton, { text: strings.Common.Close, onClick: closeDialog }))))));
};
