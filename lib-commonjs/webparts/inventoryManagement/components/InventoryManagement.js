...
class InventoryManagement extends React.Component {
    constructor(props) {
        ...
        this._clearNotification = (id, tab) => {
            const clearedKey = `${id}::${tab}`;
            const clearedNotificationIds = [...this.state.clearedNotificationIds, clearedKey];
        ...
        this._clearAllNotifications = () => {
            const notifications = this._getNotifications();
            const clearedNotificationIds = Array.from(new Set([...this.state.clearedNotificationIds, ...notifications.map(n => `${n.id}::All`)]));
            this.setState({ clearedNotificationIds });
            localStorage.setItem('inventory_cleared_notifications', JSON.stringify(clearedNotificationIds));
        };
        this._clearNotifications = (ids, tab) => {
            const clearedKeys = ids.map(id => `${id}::${tab}`);
            const clearedNotificationIds = Array.from(new Set([...this.state.clearedNotificationIds, ...clearedKeys]));
            this.setState({ clearedNotificationIds });
            localStorage.setItem('inventory_cleared_notifications', JSON.stringify(clearedNotificationIds));
        };
        ...
        this._onSendMockEmail = async () => {
            ...
            try {
                ...
                const emailService = new EmailService_1.EmailService(this.props.spContext);
                const result = await emailService.sendEmail({
                    to: recipients,
                    subject: editMockEmailSubject,
                    htmlBody: lastMockEmail.body
                });
                if (result.success) {
                    this.setState({
                        isSendingMockEmail: false,
                        mockEmailSendSuccess: true,
                        mockEmailSendMethod: result.method
                    });
                    setTimeout(() => {
                        this.setState({
                    ...
                    this.setState({
                        isSendingMockEmail: false,
                        mockEmailSendError: result.error || "Email failed to send."
                    });
                }
            }
            catch (e) {
        ...
        this._loadUserIncidentsAndReplacements = async () => {
            try {
                const activeEmail = this.state.activeUserEmail || (this.props.spContext?.pageContext?.user?.email || '');
                if (activeEmail) {
                    const incidentService = new IncidentService_1.IncidentService(this.props.spContext);
        ...
        this._onUpdateReturnRequestStatus = async (requestId, status, comment, finalCondition, adminComments, managerStatus, adminStatus) => {
            try {
                this.setState({ returnRequestsLoading: true });
                await InventoryService_1.InventoryService.updateReturnRequestStatus(requestId, status, comment, this.state.activeUserDisplayName, finalCondition, adminComments, managerStatus, adminStatus);
                await this._loadInventory();
                await this._loadReturnRequests();
                await this._loadAuditLogs();
        ...
        this._onApproveRequest = async (request) => {
            try {
                ...
                if (request.id.indexOf('temp-') === 0) {
                    this.setState(prevState => ({
                        requests: prevState.requests.map(r => r.id === request.id ? { ...r, status: 'Approved by Manager' } : r)
                    }));
                }
                else {
        ...
        this._onRejectRequest = async (request, reason) => {
            try {
                ...
                if (request.id.indexOf('temp-') === 0) {
                    this.setState(prevState => ({
                        requests: prevState.requests.map(r => r.id === request.id ? { ...r, status: 'Rejected', managerResponse: reason } : r)
                    }));
                }
                else {
        ...
        this._onApproveAsset = async (request) => {
            try {
                ...
                if (request.id.indexOf('temp-') === 0) {
                    this.setState(prevState => ({
                        requests: prevState.requests.map(r => r.id === request.id ? { ...r, assetStatus: 'Approved', status: 'Asset Assigned' } : r)
                    }));
                }
                else {
    ...
    render() {
        ...
        return (React.createElement("section", { className: `${InventoryManagement_module_scss_1.default.inventoryManagement} ${hasTeamsContext ? InventoryManagement_module_scss_1.default.teams : ''} ${isDarkTheme ? InventoryManagement_module_scss_1.default.dark : ''}` },
                    ...
                    React.createElement("div", { className: `${InventoryManagement_module_scss_1.default.sidebarContainer} ${this.state.sidebarCollapsed ? InventoryManagement_module_scss_1.default.sidebarCollapsed : ''}`, role: "navigation", "aria-label": "Main navigation" },
                        ...
                        navItems.map((nav, index) => {
                            ...
                            return (React.createElement(React.Fragment, { key: nav.key },
                                showGroupLabel && (React.createElement("div", { className: InventoryManagement_module_scss_1.default.navGroupLabel }, nav.group)),
                    ...
                    React.createElement("div", { className: `${InventoryManagement_module_scss_1.default.card} ${InventoryManagement_module_scss_1.default.contentContainer}` },
                        ...
                        (() => {
                            ...
                            switch (this.state.selectedTabKey) {
                                ...
                                case 'Notifications':
                                    return (React.createElement(NotificationCenter_1.NotificationCenter, { notifications: notifications, clearedNotificationIds: this.state.clearedNotificationIds, onMarkAsRead: this._markNotificationAsRead, onMarkAllAsRead: this._markAllNotificationsAsRead, onClearNotification: this._clearNotification, onClearAllNotifications: this._clearAllNotifications, onClearNotifications: this._clearNotifications, onNotificationAction: this._handleNotificationAction }));
            ...
            React.createElement(react_1.Panel, { isOpen: this.state.lastMockEmail !== undefined, onDismiss: () => this.setState({ lastMockEmail: undefined }), type: react_1.PanelType.medium, headerText: "\uD83D\uDCEC Outgoing Email Notification (Developer Preview)", closeButtonAriaLabel: "Close", onRenderFooterContent: () => (React.createElement(react_1.Stack, { horizontal: true, tokens: { childrenGap: 10 }, style: { padding: '10px 0' } },
                ...
                this.state.mockEmailSendSuccess && this.state.mockEmailSendMethod === "graph" && (React.createElement(react_1.MessageBar, { messageBarType: react_1.MessageBarType.success }, "Email sent via Microsoft Graph.")),
                this.state.mockEmailSendSuccess && this.state.mockEmailSendMethod === "spUtility" && (React.createElement(react_1.MessageBar, { messageBarType: react_1.MessageBarType.warning }, "Queued via legacy SharePoint mail - please verify delivery.")),
