...
class ReturnRequestService {
    ...
    static async getReturnRequests() {
        try {
            ...
            const fields = await SharePointBaseService_1.SharePointBaseService.getSafeListFields(list);
    ...
    static async addReturnRequest(request, userDisplayName) {
        ...
        try {
            ...
            schema = await SharePointBaseService_1.SharePointBaseService.getListFieldsMetadata(list);
        }
        catch (err) {
        ...
        const getField = (logicalKey, aliases) => {
            const match = SharePointBaseService_1.SharePointBaseService._resolveFieldInternalName(schema, aliases, excludeFields);
            if (match) {
        ...
        try {
            ...
            const translatedErr = SharePointBaseService_1.SharePointBaseService.translateSharePointError(err, finalPayload, resolvedMapping);
    ...
    static async cleanupReturnApprovedAssets() {
        ...
        try {
            ...
            let employeeCol = "";
            try {
                mappingList = await AssetAssignmentService_1.AssetAssignmentService.getMappingList();
                mappingFields = await SharePointBaseService_1.SharePointBaseService.getListFieldsMetadata(mappingList);
                serialCol = SharePointBaseService_1.SharePointBaseService._resolveFieldInternalName(mappingFields, ["serialnumber", "serial number"]) || "";
                assetNameCol = SharePointBaseService_1.SharePointBaseService._resolveFieldInternalName(mappingFields, ["assetname", "asset name"]) || "";
                employeeCol = SharePointBaseService_1.SharePointBaseService._resolveFieldInternalName(mappingFields, ["employee", "employee name", "employe"]) || "";
            }
            catch (e) {
