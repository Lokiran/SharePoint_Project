...
exports.AssetAssignmentService = void 0;
...
class AssetAssignmentService {
    static async getMappingList() {
        ...
        try {
            const list = sp.web.lists.getByTitle(SharePointBaseService_1.SharePointBaseService.MAPPING_LIST_NAME);
            await list.select("Title")(); // Verify list exists
            // eslint-disable-next-line require-atomic-updates
            AssetAssignmentService._resolvedMappingListName = SharePointBaseService_1.SharePointBaseService.MAPPING_LIST_NAME;
            return list;
        }
        catch (e) {
            try {
                ...
                try {
                    ...
                    await sp.web.lists.add(SharePointBaseService_1.SharePointBaseService.MAPPING_LIST_NAME, "List for tracking asset assignments", 100);
                    // eslint-disable-next-line require-atomic-updates
                    AssetAssignmentService._resolvedMappingListName = SharePointBaseService_1.SharePointBaseService.MAPPING_LIST_NAME;
                    console.log("Successfully created 'Mapping List' in SharePoint.");
                    return sp.web.lists.getByTitle(SharePointBaseService_1.SharePointBaseService.MAPPING_LIST_NAME);
                }
                catch (createError) {
                    try {
                        const allLists = await sp.web.lists.select("Title")();
                        const listNames = allLists.map(l => '"' + l.Title + '"').join(', ');
                        throw new Error("List '" + SharePointBaseService_1.SharePointBaseService.MAPPING_LIST_NAME + "' does not exist and could not be auto-created on this SharePoint site. Available lists are: [ " + listNames + " ].");
                    }
                    catch (listsError) {
                        throw new Error("List '" + SharePointBaseService_1.SharePointBaseService.MAPPING_LIST_NAME + "' does not exist and could not be auto-created.");
                    }
                }
            }
        }
    ...
    static async _writeToMappingList(employeeName, employeeId, employeeEmail, assetName, serialNumber, priority, requestedDate, reason, assignedDate, assignmentId) {
        ...
        const getField = (aliases) => {
            ...
            const match = SharePointBaseService_1.SharePointBaseService._resolveFieldInternalName(schema, aliases, mappedValues);
            return match;
        };
        ...
        let finalPayload;
        try {
            finalPayload = await SharePointBaseService_1.SharePointBaseService._coerceAndValidatePayload(logicalPayload, schema, resolvedMapping, requiredKeys);
            ...
            const translatedErr = SharePointBaseService_1.SharePointBaseService.translateSharePointError(err, finalPayload || logicalPayload, resolvedMapping);
    ...
    static async assignAssetsToEmployee(assetIds, employeeName, employeeEmail, adminName, employeeId, comment) {
        ...
        const updatePromises = assetIds.map(async (assetId) => {
            ...
            try {
                const requests = await RequestService_1.RequestService.getRequests();
            ...
            if (matchingRequest) {
                try {
                    await InventoryItemService_1.InventoryItemService.updateAssetStatus(parseInt(matchingRequest.id, 10), 'Approved', adminName, comment);
                }
                catch (err) {
    ...
    static async syncExistingAssignmentsToMappingList(adminName) {
        ...
        try {
            ...
            const list = await InventoryItemService_1.InventoryItemService.getInventoryList();
            const items = await InventoryItemService_1.InventoryItemService.getItems();
            // Filter for assigned items matching the 5 active employees
            const assignedItems = items.filter(item => {
                return mockData_1.EMPLOYEES.some(emp => {
            ...
            for (const asset of assignedItems) {
                ...
                const employee = mockData_1.EMPLOYEES.find(emp => {
                    const normalize = (value) => (value || '').toLowerCase().replace(/[^a-z0-9]/g, '');
                    const activeUser = normalize(emp.name);
                    if (!activeUser)
                        return false;
                ...
                if (matchingRequest && matchingRequest.assetStatus === 'Pending') {
                    try {
                        await InventoryItemService_1.InventoryItemService.updateAssetStatus(parseInt(matchingRequest.id, 10), 'Approved', adminName);
                    }
                    catch (err) {
                        console.warn(`Failed to update assetStatus to Approved for Request ${matchingRequest.id}`, err);
                    }
