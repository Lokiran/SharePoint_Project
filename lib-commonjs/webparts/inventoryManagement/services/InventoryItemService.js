...
class InventoryItemService {
    static async getInventoryList() {
        ...
        try {
            const list = sp.web.lists.getByTitle(SharePointBaseService_1.SharePointBaseService.LIST_NAME);
            await list.select("Title")(); // Verify list exists
            // eslint-disable-next-line require-atomic-updates
            InventoryItemService._resolvedListName = SharePointBaseService_1.SharePointBaseService.LIST_NAME;
            return list;
        }
        catch (e) {
            try {
                ...
                try {
                    ...
                    throw new Error("List '" + SharePointBaseService_1.SharePointBaseService.LIST_NAME + "' or 'Inventory List' does not exist on this SharePoint site. Available lists on this site are: [ " + listNames + " ]. Please ensure your list title matches exactly.");
                }
                catch (listsError) {
                    throw new Error("List '" + SharePointBaseService_1.SharePointBaseService.LIST_NAME + "' or 'Inventory List' does not exist on this SharePoint site.");
                }
            }
        }
    }
    ...
    static async updateAssetStatus(requestId, assetStatus, approverName = 'Unknown', comment) {
        try {
            ...
            let list;
            try {
                list = sp.web.lists.getByTitle(SharePointBaseService_1.SharePointBaseService.REQUEST_LIST_NAME);
            ...
            const fields = await list.fields.select("InternalName", "Title", "TypeAsString")();
            const hasRequestStatus = fields.some(field => (field.InternalName || '').toString().toLowerCase() === SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME.toLowerCase());
            const hasManagerComment = fields.some(field => (field.InternalName || '').toString().toLowerCase() === SharePointBaseService_1.SharePointBaseService.REQUEST_COMMENT_INTERNAL_NAME.toLowerCase());
            ...
            const hasAssetStatus = fields.some(field => (field.InternalName || '').toString().toLowerCase() === SharePointBaseService_1.SharePointBaseService.ASSET_STATUS_INTERNAL_NAME.toLowerCase());
