    ...
    static async getRequestList() {
        ...
        try {
            ...
            RequestService._resolvedRequestListName =
                SharePointBaseService_1.SharePointBaseService.REQUEST_LIST_NAME;
            return list;
        }
        catch (e) {
            try {
        ...
        try {
            ...
            const hasRequestStatus = fields.some(field => {
                const internalName = (field.InternalName || "")
                    ...
                    .toLowerCase();
                return (internalName ===
                    SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME.toLowerCase());
            });
            const hasManagerComment = fields.some(field => {
                const internalName = (field.InternalName || "")
                    ...
                    .toLowerCase();
                return (internalName ===
                    SharePointBaseService_1.SharePointBaseService.REQUEST_COMMENT_INTERNAL_NAME.toLowerCase());
            });
            if (!hasRequestStatus) {
                try {
            ...
            const hasAssetStatus = fields.some(field => {
                const internalName = (field.InternalName || "")
                    ...
                    .toLowerCase();
                return (internalName ===
                    SharePointBaseService_1.SharePointBaseService.ASSET_STATUS_INTERNAL_NAME.toLowerCase());
            });
            if (!hasAssetStatus) {
                try {
    ...
    static async addRequest(request, userDisplayName = "Unknown", userRole, isEmployeeUI) {
        ...
        try {
            ...
            if (statusField) {
                ...
                console.warn("RequestStatus field was not detected. Using configured fallback internal name:", SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME);
                payload[SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME] = initialStatus;
            }
            /*
             * Priority
             */
            ...
            if (!Number.isNaN(requestItemId)) {
                try {
                    ...
                    const hasAssetStatusField = fields.some((field) => (field.InternalName || "").toLowerCase() ===
                        SharePointBaseService_1.SharePointBaseService
                            .ASSET_STATUS_INTERNAL_NAME
                            .toLowerCase());
                    if (hasAssetStatusField) {
                        updatePayload[SharePointBaseService_1.SharePointBaseService
    ...
    static async updateRequestStatus(requestId, status, approverName = "Unknown", rejectionReason) {
        try {
            ...
            const statusKey = statusKeyFromItem ||
                ...
                SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME;
            if (!statusKey) {
            ...
            const reasonKey = findKey("managercomment") ||
                SharePointBaseService_1.SharePointBaseService.REQUEST_COMMENT_INTERNAL_NAME ||
