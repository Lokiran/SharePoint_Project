...
class RequestService {
    ...
    static _resolveRequestKeyInternalName(fields) {
        ...
        return SharePointBaseService_1.SharePointBaseService.REQUEST_KEY_INTERNAL_NAME;
    }
    ...
    static async getRequestList() {
        ...
        try {
            const list = sp.web.lists.getByTitle(SharePointBaseService_1.SharePointBaseService.REQUEST_LIST_NAME);
            await list.select("Title")(); // Verify list exists
            // eslint-disable-next-line require-atomic-updates
            RequestService._resolvedRequestListName = SharePointBaseService_1.SharePointBaseService.REQUEST_LIST_NAME;
            return list;
        }
        catch (e) {
            try {
                ...
                try {
                    ...
                    throw new Error("List '" + SharePointBaseService_1.SharePointBaseService.REQUEST_LIST_NAME + "' or 'Request List' does not exist on this SharePoint site. Available lists are: [ " + listNames + " ].");
                }
                catch (listsError) {
                    throw new Error("List '" + SharePointBaseService_1.SharePointBaseService.REQUEST_LIST_NAME + "' or 'Request List' does not exist.");
                }
            }
        }
    }
    ...
    static async _ensureRequestWorkflowFields() {
        ...
        try {
            ...
            const hasRequestStatus = fields.some(field => {
                ...
                return internalName === SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME.toLowerCase();
            });
            const hasManagerComment = fields.some(field => {
                const internalName = (field.InternalName || '').toString().toLowerCase();
                return internalName === SharePointBaseService_1.SharePointBaseService.REQUEST_COMMENT_INTERNAL_NAME.toLowerCase();
            });
            if (!hasRequestStatus) {
                try {
                    await list.fields.addChoice(SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME, {
            ...
            if (!hasManagerComment) {
                try {
                    await list.fields.addMultilineText(SharePointBaseService_1.SharePointBaseService.REQUEST_COMMENT_INTERNAL_NAME);
                }
                catch (err) {
            ...
            if (!hasRequestKey) {
                try {
                    await list.fields.addText(SharePointBaseService_1.SharePointBaseService.REQUEST_KEY_INTERNAL_NAME);
                }
                catch (err) {
            ...
            const hasAssetStatus = fields.some(field => {
                const internalName = (field.InternalName || '').toString().toLowerCase();
                return internalName === SharePointBaseService_1.SharePointBaseService.ASSET_STATUS_INTERNAL_NAME.toLowerCase();
            });
            if (!hasAssetStatus) {
                try {
                    await list.fields.addChoice(SharePointBaseService_1.SharePointBaseService.ASSET_STATUS_INTERNAL_NAME, {
    ...
    static async addRequest(request, userDisplayName = "Unknown") {
        ...
        try {
            ...
            if (statusField) {
                ...
                dynamicPayload[SharePointBaseService_1.SharePointBaseService.REQUEST_STATUS_INTERNAL_NAME] = initialStatus;
            }
            if (employeeIdField) {
            ...
            if (!Number.isNaN(requestItemId)) {
                try {
                    ...
                    await requestListInstance.items.getById(requestItemId)
                        .update({
                        [resolvedKeyName]: requestKey,
                        [SharePointBaseService_1.SharePointBaseService.ASSET_STATUS_INTERNAL_NAME]: "Pending"
                    });
                }
                catch (err) {
    ...
    static async getRequests() {
        try {
            ...
            const list = await RequestService.getRequestList();
            const fields = await list.fields.select("InternalName", "Title", "TypeAsString")();
            const items = await SharePointBaseService_1.SharePointBaseService._fetchItemsWithExpandedUsers(list);
    ...
    static async updateRequestStatus(requestId, status, approverName = 'Unknown', rejectionReason) {
        try {
            ...
            if (!SharePointBaseService_1.SharePointBaseService._isBusinessStatusKey(statusKey)) {
                throw new Error('Detected non-business status field. Please ensure RequestList has a dedicated request status column.');
            }
            const reasonKey = findKey("managercomment") || SharePointBaseService_1.SharePointBaseService.REQUEST_COMMENT_INTERNAL_NAME || findKey("rejectionreason") || findKey("comments") || findKey("reason");
    ...
    static async getRequestHistoryById(requestLookupId) {
        ...
        const allLogs = await AuditLogService_1.AuditLogService.getAuditLogs();
